...
/* Includes ---------------------------------------------------------------- */
#include <Electronic_components_classification_inferencing.h>
...
void loop()
{
    ...
    // Initialize variables to track the class with the highest probability
    uint16_t maxIndex = 0;
    float maxProb = result.classification[0].value;

    // Loop through the classifications to find the class with the highest probability
    for (uint16_t i = 1; i < EI_CLASSIFIER_LABEL_COUNT; i++) {
        if (result.classification[i].value > maxProb) {
            maxProb = result.classification[i].value;
            maxIndex = i;
        }
    }

    // Print only the class with the highest probability
    ei_printf("  %s: %.5f\r\n", ei_classifier_inferencing_categories[maxIndex], maxProb);
#endif
